...
class CliRunTest < Minitest::Test
  ...
  def test_creates_file_only_daily
    ...
    in_mock_time_jail(timelock, ssh_mock) do
      ...
      [:monthly, :weekly, :yearly].each do |s|
        expected_file_path = File.join('backups/def', s.to_s, '20190202_0000.tgz')
  ...
  def test_creates_file_seed_others
    timelock = Date.civil(2019,2,2)
    ssh_mock = "touch backups/def/daily/2019-02-02_0000.tgz"
    ...
    in_mock_time_jail(timelock, ssh_mock) do
      ...
      [:daily, :monthly, :weekly, :yearly].each do |s|
        expected_file_path = File.join('backups/def', s.to_s, '2019-02-02_0000.tgz')
        assert File.exist?(expected_file_path), "backup file in '#{s}' should exist"
      end
